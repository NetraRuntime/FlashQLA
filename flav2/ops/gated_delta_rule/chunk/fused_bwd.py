...
def tilelang_fused_chunk_gdr_bwd(
    ...
    def tilelang_fused_chunk_gdr_bwd_kernel(
        ...
        with T.Kernel(batch_size * H, threads=512) as (bbh, ):
            ...
            odot_fragment_3 = T.alloc_fragment((DK, DV), dtype=accum_dtype)
            reduce_fragment = T.alloc_fragment((128, 2), dtype=accum_dtype)
            dg_last_local_3 = T.alloc_fragment((1), dtype=accum_dtype)

            # 16 stages
            bar_dht = T.alloc_barrier(arrive_count=128)
            bar_dk = T.alloc_barrier(arrive_count=128)
            bar_00 = T.alloc_barrier(arrive_count=320)
            bar_01 = T.alloc_barrier(arrive_count=384)
            bar_02 = T.alloc_barrier(arrive_count=320)
            ...
            bar_15 = T.alloc_barrier(arrive_count=288)
            ...
            T.annotate_layout({
                ...
                tmp_shared_1_1: tilelang.layout.make_swizzled_layout(tmp_shared_1_1),
                tmp_shared_1_2: tilelang.layout.make_swizzled_layout(tmp_shared_1_2),
                tmp_shared_2_1: tilelang.layout.make_swizzled_layout(tmp_shared_2_1),
                tmp_shared_2_2: tilelang.layout.make_swizzled_layout(tmp_shared_2_2),
                tmp_shared_2_3: tilelang.layout.make_swizzled_layout(tmp_shared_2_3),
                tmp_shared_4_1: tilelang.layout.make_swizzled_layout(tmp_shared_4_1),
            })
            ...
            T.copy(h[batch_idx, chunk_start_idx + num_iters - 1, bh, 0:DK, 0:DV], h_shared)
            ...
            if tx >= PRODUCER_WG_IDX * 128 and tx < (PRODUCER_WG_IDX + 1) * 128:
                ...
                if tx < PRODUCER_WG_IDX * 128 + 32:
                    ...
                    for i_s in T.serial(num_iters - 1):
                        ...
                        T.barrier_wait(bar_05, (i_s + 2) % 2)
                        T.copy(v[batch_idx, left:right, bh, 0:DV], v_shared)

                        T.barrier_wait(bar_07, (i_s + 2) % 2)
                        T.copy(k[batch_idx, left:right, bhg, 0:DK], k_shared)

                        T.barrier_wait(bar_09, (i_s + 2) % 2)
                        T.copy(q[batch_idx, left:right, bhg, 0:DK], q_shared)
                        T.barrier_arrive(bar_00)

                        T.barrier_wait(bar_10, (i_s + 2) % 2)
                        T.copy(h[batch_idx, chunk_start_idx + chunk_idx, bh, 0:DK, 0:DV], h_shared)

                        T.barrier_wait(bar_14, (i_s + 2) % 2)
                    ...
                    for i_s in T.serial(num_iters - 1):
                        left = seq_start_idx + (num_iters - i_s - 2) * block_S

                        T.barrier_wait(bar_03, (i_s + 2) % 2)
                        for j_s in T.Parallel(block_S):
                            g_shared[j_s] = g[batch_idx, left + j_s, bh]
                        T.barrier_arrive(bar_00)

                        T.barrier_wait(bar_15, (i_s + 2) % 2)
                        for j_s in T.Parallel(block_S):
                    ...
                    for j_s in T.Parallel(block_S):
                        if seq_start_idx + (num_iters - 1) * block_S + j_s < seq_end_idx:
                            dg[batch_idx, seq_start_idx + (num_iters - 1) * block_S + j_s, bh] = dg_shared[j_s]
                    ...
                    for j_s in T.Parallel(block_S):
                        if seq_start_idx + (num_iters - 1) * block_S + j_s < seq_end_idx:
                            db[batch_idx, seq_start_idx + (num_iters - 1) * block_S + j_s, bh] = db_shared[j_s]
                    ...
                    for j_s, j_k in T.Parallel(block_S, DK):
                        if seq_start_idx + (num_iters - 1) * block_S + j_s < seq_end_idx:
                            dv[batch_idx, seq_start_idx + (num_iters - 1) * block_S + j_s, bh, j_k] = dqkv_shared[j_s, j_k]
                    T.barrier_arrive(bar_10)
                    
                    T.barrier_wait(bar_11, 0)
                    for j_s, j_k in T.Parallel(block_S, DK):
                        if seq_start_idx + (num_iters - 1) * block_S + j_s < seq_end_idx:
                            dq[batch_idx, seq_start_idx + (num_iters - 1) * block_S + j_s, bh, j_k] = dqkv_shared[j_s, j_k]
                    T.barrier_arrive(bar_15)

                    if num_iters > 1:
                        T.barrier_wait(bar_00, 1)
                    else:
                        T.barrier_wait(bar_dk, 0)
                    for j_s, j_k in T.Parallel(block_S, DK):
                        if seq_start_idx + (num_iters - 1) * block_S + j_s < seq_end_idx:
                            dk[batch_idx, seq_start_idx + (num_iters - 1) * block_S + j_s, bh, j_k] = dqkv_shared[j_s, j_k]
                    ...
                    for i_s in T.serial(num_iters - 1):
                        ...
                        if i_s > 0:
                            T.barrier_wait(bar_00, (i_s + 1) % 2)
                            T.copy(dqkv_shared, dk[batch_idx, left+block_S:right+block_S, bh, 0:DK])
                        T.barrier_arrive(bar_04)

                        T.barrier_wait(bar_05, (i_s + 1) % 2)
                        T.copy(dqkv_shared, dv[batch_idx, left:right, bh, 0:DV])
                        T.barrier_arrive(bar_10)

                        T.barrier_wait(bar_11, (i_s + 1) % 2)
                        T.copy(dqkv_shared, dq[batch_idx, left:right, bh, 0:DK])
                        T.barrier_arrive(bar_15)

                    if num_iters > 1:
                        T.barrier_wait(bar_dk, 0)
                        T.copy(dqkv_shared, dk[batch_idx, seq_start_idx:seq_start_idx+block_S, bh, 0:DK])

            elif tx >= CONSUMER_K_WG_IDX * 128 and tx < (CONSUMER_K_WG_IDX + 1) * 128:
                T.set_max_nreg(CONSUMER_K_NREG, 1)

                T.barrier_wait(bar_dht, 0)
                ...
                for i_s in T.serial(num_iters):
                    ...
                    # dV' += Pg^T @ dO
                    T.gemm_v1(tmp_shared_1_1, do_shared, dv_fragment, transpose_A=True, clear_accum=False)
                    ...
                    # dV = Ag^T @ dV'
                    T.gemm_v1(tmp_shared_1_2, tmp_shared_2_1, dv_fragment, transpose_A=True, clear_accum=True)
                    # S2[S] dV
                    ...
                    # dVg = -g * dV
                    for j_s, j_v in T.Parallel(block_S, DV):
                        dv_fragment[j_s, j_v] = - dv_fragment[j_s, j_v] * g_exp_shared[j_s]
                    # dg += sum(dVg * U)
                    for j_s, j_v in T.Parallel(block_S, DV):
                        odot_fragment_1[j_s, j_v] = dv_fragment[j_s, j_v] * tmp_shared_2_3[j_s, j_v]
                    ...
                    # dK = V' @ dSt^T
                    T.gemm_v1(tmp_shared_2_1, tmp_shared_4_1, dk_fragment, transpose_B=True, clear_accum=True)
                    ...
                    # dg -= sum(K * dK)
                    for j_s, j_k in T.Parallel(block_S, DK):
                        odot_fragment_1[j_s, j_k] = - tmp_shared_2_2[j_s, j_k] * dk_fragment[j_s, j_k]
                    T.reduce_sum(odot_fragment_1, dg_fragment_1, dim=1, clear=True)
                    for j_s in T.Parallel(block_S):
                    ...
                    # dK += dVg @ S0^T
                    T.gemm_v1(tmp_shared_2_3, h_shared, dk_fragment, transpose_B=True, clear_accum=False)
                    T.barrier_arrive(bar_10)

                    # 12
                    T.barrier_wait(bar_12, (i_s + 0) % 2)
                    # dK += dP^T @ Q
                    T.gemm_v1(tmp_shared_1_1, tmp_shared_2_1, dk_fragment, transpose_A=True, clear_accum=False)
                    T.barrier_arrive(bar_13)

                    # 15
                    T.barrier_wait(bar_15, (i_s + 0) % 2)
                    # dK += dAs @ K
                    T.gemm_v1(tmp_shared_1_2, tmp_shared_2_2, dk_fragment, clear_accum=False)
                    T.copy(dk_fragment, dqkv_shared)

                T.barrier_arrive(bar_dk)

            elif tx >= CONSUMER_A_WG_IDX * 128 and tx < (CONSUMER_A_WG_IDX + 1) * 128:
                ...
                for i_s in T.serial(num_iters):
                    ...
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        with T.If(j_s >= j_t):
                            with T.Then():
                                mask_fragment[j_s, j_t] = T.exp2((g_shared[j_s] - g_shared[j_t]) * 1.442695)
                            with T.Else():
                                mask_fragment[j_s, j_t] = 0
                    # Pg = s * P * G
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        a_fragment[j_s, j_t] = scale * p_fragment[j_s, j_t] * mask_fragment[j_s, j_t]
                    # S1[1] Pg
                    ...
                    # Ab = Ar * b
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        a_fragment[j_s, j_t] = a_shared[j_s, j_t] * b_shared[j_t]
                    # Ag = G * Ab
                    for j_s, j_t in T.Parallel(block_S, block_S):
                    ...
                    # W = V - g * U
                    for j_s, j_v in T.Parallel(block_S, DV):
                        u_fragment[j_s, j_v] = v_shared[j_s, j_v] - g_exp_shared[j_s] * u_fragment[j_s, j_v]
                    # S2[2] W
                    ...
                    T.barrier_wait(bar_05, (i_s + 0) % 2)
                    # dAg = dV' @ W^T
                    T.gemm_v1(tmp_shared_2_1, tmp_shared_2_2, da_fragment, transpose_B=True, clear_accum=True)
                    # V' = Ag @ W
                    T.gemm_v1(tmp_shared_1_2, tmp_shared_2_2, u_fragment, clear_accum=True)
                    # S2[1] V'
                    ...
                    T.barrier_wait(bar_06, (i_s + 0) % 2)
                    # dPg = dO @ V'^T
                    T.gemm_v1(do_shared, tmp_shared_2_1, dp_fragment, transpose_B=True, clear_accum=True)
                    T.barrier_arrive(bar_07)
                    ...
                    # dg += sum(dPg * P)
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        p_fragment[j_s, j_t] = tmp_shared_1_1[j_s, j_t] * dp_fragment[j_s, j_t]
                    ...
                    T.reduce_sum(p_fragment, dg_fragment_2, dim=1, clear=False)
                    # dP = s * G * dPg
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        dp_fragment[j_s, j_t] *= scale * mask_fragment[j_s, j_t]
                    # S1[1] dP
                    ...
                    T.barrier_wait(bar_08, (i_s + 0) % 2)
                    # dQ = dO @ S0^T
                    T.gemm_v1(do_shared, h_shared, dq_fragment, transpose_B=True, clear_accum=True)
                    T.barrier_arrive(bar_09)
                    ...
                    # dQ = s * g * dQ
                    for j_s, j_k in T.Parallel(block_S, DK):
                        dq_fragment[j_s, j_k] *= scale * g_exp_shared[j_s]
                    # dg += sum(Q * dQ)
                    for j_s, j_k in T.Parallel(block_S, DK):
                        odot_fragment_2[j_s, j_k] = tmp_shared_2_1[j_s, j_k] * dq_fragment[j_s, j_k]
                    ...
                    # dQ += dP @ K
                    T.gemm_v1(tmp_shared_1_1, tmp_shared_2_2, dq_fragment, clear_accum=False)
                    # S2[S] dQ
                    ...
                    # dg += sum(dAg * Ag)
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        a_fragment[j_s, j_t] = da_fragment[j_s, j_t] * tmp_shared_1_2[j_s, j_t]
                    ...
                    # db = sum(dAb * Ar)
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        a_fragment[j_s, j_t] = a_shared[j_s, j_t] * da_fragment[j_s, j_t]
                    T.copy(a_fragment, tmp_shared_1_2)
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        a_fragment[j_s, j_t] = tmp_shared_1_2[j_t, j_s]
                    ...
                    # dA = -Ar^T @ dAr @ Ar^T
                    T.gemm_v1(a_shared, tmp_shared_1_2, da_fragment, transpose_A=True, clear_accum=True)
                    T.copy(da_fragment, tmp_shared_1_2)
                    T.gemm_v1(tmp_shared_1_2, a_shared, da_fragment, transpose_B=True, clear_accum=True)
                    # At = K @ K^T
                    T.gemm_v1(tmp_shared_2_2, tmp_shared_2_2, a_fragment, transpose_B=True, clear_accum=True)
                ...
                for i_s in T.serial(num_iters):
                    ...
                    for j_s in T.Parallel(block_S):
                        ...
                        g_rev_exp_shared[j_s] = T.exp2((g_shared[block_S - 1] - g_shared[j_s]) * 1.442695)
                    T.barrier_arrive(bar_01)
                    ...
                    for j_k, j_v in T.Parallel(DK, DV):
                        dh_fragment[j_k, j_v] *= g_exp_shared[block_S - 1]
                    T.barrier_arrive(bar_04)
                    ...
                    for j_k, j_v in T.Parallel(DK, DV):
                        reduce_fragment[j_k % 64 // 16 * 32 + j_k % 8 * 4 + j_v % 8 // 2, j_v % 2] += \
                    ...
                    T.barrier_wait(bar_08, (i_s + 0) % 2)
                    # S2[1] Q
                    T.copy(q_shared, tmp_shared_2_1)
                    T.barrier_arrive(bar_09)
                    ...
                    T.barrier_wait(bar_11, (i_s + 0) % 2)
                    # dS0 += K^T @ dVg
                    T.gemm_v1(tmp_shared_2_2, tmp_shared_2_3, dh_fragment, transpose_A=True, clear_accum=False)
                    ...
                    # dOg = s * g * dO
                    for j_s, j_v in T.Parallel(block_S, DV):
                        tmp_shared_2_3[j_s, j_v] = scale * do_shared[j_s, j_v] * g_exp_shared[j_s]
                    T.barrier_arrive(bar_14)

                    # 14
                    T.barrier_wait(bar_14, (i_s + 0) % 2)
                    # dS0 += Q^T @ dOg
                    T.gemm_v1(tmp_shared_2_1, tmp_shared_2_3, dh_fragment, transpose_A=True, clear_accum=False)
                    T.barrier_arrive(bar_15)
...
def fused_gdr_bwd(
    ...
    if dht is None:
        dht = torch.empty((real_batch_size, H, K, V), dtype=torch.float32, device=k.device)
    ...
    tilelang_fused_chunk_gdr_bwd_kernel = tilelang_fused_chunk_gdr_bwd(
        ...
        is_varlen=is_varlen, use_dht=use_dht,
        **dtypes,
    )
    tilelang_fused_chunk_gdr_bwd_kernel(
        do, dht, q, k, v, a, g, b, h,
